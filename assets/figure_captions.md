...
**Figure 2.** Aggregated confusion matrix for BioBERT across all 5 folds (total 56 predictions). Rows represent true labels (Solo = single trophic mode, Dual = multiple trophic modes), columns represent predicted labels. BioBERT achieved balanced performance with similar error rates for both classes (6 solo samples misclassified as dual, 6 dual samples misclassified as solo), demonstrating no systematic bias toward either category. Color intensity indicates prediction frequency; diagonal cells (correct predictions) show 22 solo and 22 dual correct classifications. Source: [figures/confusion_matrices_comparison.png](../figures/confusion_matrices_comparison.png)
